                ...
                context "when the expression is true" $ do
                    context "when the block does not contain errors" $ do
                        it "executes the block and returns OK" $ do
                            let ifStatement = IfStatement (BooleanValue True) printStatementsBlock1 NoOp
