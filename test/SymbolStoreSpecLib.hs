module SymbolStoreSpecLib ( mutableSymTablesWith
                          , mutableSymTablesWithout
                          , immutableSymTablesWith
                          , immutableSymTablesWithout
                          ) where
...
mutableSymTablesWithout :: Gen (Symbol, MutableSymbolTable Symbol Expression)
mutableSymTablesWithout = do
    s <- genSymbol
    m <- suchThat genMapOrEmptyMap (s `M.notMember`)
    return (s, MutableSymbolTable m)

mutableSymTablesWith :: Gen (Symbol, Expression, MutableSymbolTable Symbol Expression)
mutableSymTablesWith = do
    s <- genSymbol
    m <- genMapOrEmptyMap
    e <- genExpression
    let mm = M.insert s e m
    return (s, e, MutableSymbolTable mm)

immutableSymTablesWithout :: Gen (Symbol, ImmutableSymbolTable Symbol Expression)
immutableSymTablesWithout = do
    s <- genSymbol
    m <- suchThat genMapOrEmptyMap (s `M.notMember`)
    return (s, ImmutableSymbolTable m)

immutableSymTablesWith :: Gen (Symbol, Expression, ImmutableSymbolTable Symbol Expression)
immutableSymTablesWith = do
    s <- genSymbol
    m <- genMapOrEmptyMap
    e <- genExpression
    let mm = M.insert s e m
    return (s, e, ImmutableSymbolTable mm)

-- shouldHaveSymbolValue :: (SymbolStore ss) => ss -> (Symbol, Expression) -> Bool
-- shouldHaveSymbolValue ss (symbol, value) =
--     case getSymbolValue symbol ss of
--         Left _  -> False
--         Right x ->
...
genMapOrEmptyMap :: Gen (M.Map Symbol Expression)
genMapOrEmptyMap = oneof [ genMap, genEmptyMap ]

genEmptyMap :: Gen (M.Map Symbol Expression)
genEmptyMap = return M.empty

genMap :: Gen (M.Map Symbol Expression)
genMap = do
    sz <- suchThat arbitrary (\x -> x >= 1 && x <= 60) :: Gen Int
